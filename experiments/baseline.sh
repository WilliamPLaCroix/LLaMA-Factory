...
RUN_KEY="${MODEL_VARIATION}-${BASE_GROUP}-v1-4"
...
export WANDB_NAME="${MODEL_VARIATION}-${BASE_GROUP}-4"           # stable name per train variant
...
for DATASET_VARIATION in "${ds_variations[@]}"; do
  ...
  for grade in {02..12}; do
    ...
    export WANDB_RUN_ID
    export WANDB_RESUME=allow
    export WANDB_NAME="${MODEL_VARIATION}-${BASE_GROUP}-4"   # keep stable name for color-by-run
