...
#custom code start here------
        if len(adapter_to_merge) > 1 and model_args.combination_type is not None:
            ...
            logger.info_rank0(f"Merged {len(adapter_to_merge)} adapter(s) into base model. (First, merged adapters with weights {weights} and combination_type of {model_args.combination_type}, then merged into base model)")
